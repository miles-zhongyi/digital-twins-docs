...
- **UE ↔ RU (the "Uu" link):** one TCP socket *per UE*. In `ru/ru_server.py`,
...
- **RU ↔ DU (the "F1" link):** exactly **one** multiplexed TCP connection per RU,
...
peer went away." Because every UE→DU uplink elicits exactly one DU→UE downlink, the
RU can be a synchronous transparent proxy: receive from the UE, stamp RF/cell
...
- `ATTACH_FLOW` — `RRC_CONNECTION_REQUEST → RRC_CONNECTION_SETUP`, `..._SETUP_COMPLETE
  ...
  `ACT_ADMIT` — the PRB grant decision happens here), `RECONFIGURATION_COMPLETE →
...
`m_tmsi`, S1AP ids, the canonical `record_id`, the cosmetic numeric `cell_id`), and
...
ASN.1-shaped body and any opaque hex blobs, is preserved verbatim from the sample,
...
- **DU** (`du/du_server.py`) — one `Cell` (PRB pool, `used_prbs`/`total_prbs`)
  ...
  (for flow steps with no capacity action, like RRC setup or security mode). All
