...
Dashboard: `http://localhost:9090`. DU JSON status: `http://localhost:9080/status`
...
Trace replay drives UE attach/measurement/release timing from real decoded call
...
`TRAFFIC_PROFILE` (env var on both `du` and `ue-sim`) selects how PRBs are sized
...
RU at the origin, and the UE simulator directly with `python3`, setting
...
- **No 5G NR signalling, despite the project's "5G RU Digital Twin" name.**
  ...
  catalog is `LteCatalog` (RRC + S1AP, eNB/MME/S1 naming). `RADIO_TECH=nr` is a
  ...
  The nr.py docstring notes the intended NR flow (NR-RRC over Uu, NGAP toward the
  AMF: RRC Setup, Registration Request, Authentication, Security Mode, UE
...
- **`docker-compose.yml`'s `TOTAL_PRBS=250` overrides a different in-code default
  ...
  environment override silently changes per-cell capacity.
